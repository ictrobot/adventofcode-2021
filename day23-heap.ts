...
function solve(input: string[], roomYs: number[]) {
    ...
    const queue = new Heap<[state: string, cost: number, minCost: number]>((x, y) => x[2] - y[2]);
    queue.push([positions.map(p => input[p.y][p.x]).join(''), 0, 0]);
    ...
    while (!queue.empty()) {
        ...
        for (const r of possibleMoves(s, c)) queue.push(r);
    }
}
